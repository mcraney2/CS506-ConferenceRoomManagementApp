...
test('Repeat Selection dropdown sends repeat  to handler', () => {
    const onEventMock = jest.fn();
    const {getByTestId} = render(
        <RepeatSelectDropdown setRepeat = {onEventMock}/>
    );
    fireEvent(getByTestId('repeat'),'onValueChange', 'Weekly');
    expect(onEventMock).toHaveBeenCalledWith('Weekly');
    fireEvent(getByTestId('repeat'),'onValueChange', 'Monthly');
    expect(onEventMock).toHaveBeenCalledWith('Monthly');
    fireEvent(getByTestId('repeat'),'onValueChange', 'Never');
    expect(onEventMock).toHaveBeenCalledWith('Never');
});
...
test('Repeat handler sets the state of the event', () => {
    Enzyme.configure({adapter: new Adapter()})
    const test = shallow(<AdminAddEvent/>);
    const instance = test.instance();
    instance.setRepeat('Weekly');
